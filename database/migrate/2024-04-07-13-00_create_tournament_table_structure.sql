...
CREATE TABLE IF NOT EXISTS ko_matches (
    ...
    group_id INT NOT NULL,
    group_id1 INT,
    -- can be null when the value depends on the outcome of previous matches
    group_id2 INT,
    -- can be null when the value depends on the outcome of previous matches
...
CREATE TABLE IF NOT EXISTS thirds_evaluation_rules (
    tournament_id INT,
    best_four_teams_id INT,
    best_four_teams_arrangement INT [4],
    FOREIGN KEY (tournament_id) REFERENCES tournaments(id)
);
